...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int64_t)src1 >> imm);
  ...
  INSTPAT("0000000 0???? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT(src1 << imm, 32));
  INSTPAT("0000000 0???? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(BITS(src1, 31, 0) >> imm, 32));
  INSTPAT("0100000 0???? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((int32_t)BITS(src1, 31, 0) >> imm, 32));
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(rd) = SEXT(BITS(src1, 31, 0) + BITS(src2, 31, 0), 32));
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , R, R(rd) = SEXT(BITS(src1, 31, 0) - BITS(src2, 31, 0), 32));
