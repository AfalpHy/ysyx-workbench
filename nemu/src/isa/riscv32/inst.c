...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; Assert(s->dnpc % 4 == 0, "target address is not aligned to a four-byte boundary"));
  
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & ~1; Assert(s->dnpc % 4 == 0, "target address is not aligned to a four-byte boundary"));
